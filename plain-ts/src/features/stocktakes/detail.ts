...
import { difference, editBlockReason, isEditable, statusLabel } from '../../domain/rules.ts';
...
export function renderStocktakeDetail(
    ...
    return el(
      ...
      el(
        'button',
        {
          ...
          onclick: () => toast('Line editor — coming in Stage 4.', 'info'),
        },
        icon('plus', 18),
        'Add item',
      ),
  ...
  function renderLineToolbar(_s: Stocktake, _editable: boolean): Node {
    return el(
      'div',
      { class: 'line-toolbar' },
  ...
  function renderLineTable(): Node {
    ...
    return table<StocktakeLine>({
      ...
      rows: lines(),
      rowKey: (l) => l.id,
      onRowClick: () => toast('Line editor — coming in Stage 4.', 'info'),
  ...
  function renderStatusRegion(s: Stocktake, editable: boolean): Node {
    ...
    const finaliseControl = editable
      ? splitButton({
          ...
          onPrimary: () => toast('Finalise — coming in Stage 4.', 'info'),
        })
      : null;
