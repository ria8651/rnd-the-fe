...
- **State lives in the URL.** Every filter binds to a named URL query parameter, so the filtered view is **shareable, bookmarkable, and survives reload / back-forward**. Reading initial filter state from the URL means a link can open a list pre-filtered. Parameter naming, value encoding, and history behaviour follow the shared [URL & navigation state](./urls.md) rules.
